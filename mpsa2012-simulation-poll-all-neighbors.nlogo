...
globals
[
  value-list ;;possible values: "political equality" "conservatism" "group dominance"
  total-approval ;; total # of approving turtles
  pct-approval ;; percentage for convenience

  ;;Whether an information shock has been applied, either TRUE or FALSE
  info-shock?
  
  ;;global value to keep track of how many people had changed preferences
  num-changed

;;global variables keeping track of people who reconsider their opinions but (1) change their minds or (2) do not  
reconsider-change
reconsider-not-change
  
  ;;boolean for cleaning up
  clean-up?
]
...
to setup
  ...
  if num-agents > count patches
    [ user-message (word "This pond only has room for " count patches " turtles.") 
      ;; should never happen as number is limited to max number of patches (default is 1089), but you never know
      stop ]
  ...
  ;; just to clear the time counter 
  reset-ticks
end
...
to make-attributes
  ...
  set age 0 ;; initially we set age to an out of bound value, then assign it some age
  ...
  while [random-jitter < -0.5 OR random-jitter > 0.2 ] [set random-jitter random-normal -0.1 0.2]

  ;; abortion for health regression regression for determining initial opinion-level
  ...
  set-bool max-value ;; determine what the person values most with regard to values (conservatism, equality, etc.) Placeholder that takes on more significance in v2, in which a turtle changes his mind only if likeminded (read: people who share the same priorities as he) persuade him otherwise.
...
;;In this version, it is not used, as the turtle will take into account the preferences of ALL neighbors
...
to update-turtles ;by observer
  ...
  ask turtles [  
    if ( max-value-weight - second-value-weight ) < threshold-tolerance ;; totally arbitrary value, we're not sure how to interpret this, really
    [ 
      set reconsider? TRUE ;; in case I want to count how many people "reconsider" later
      
      ;;count total number of neighbors
      
      set neighbors-num count (turtles-on neighbors)
      
      ;;count number of people who approve
      set total-agree count (turtles-on neighbors) with [ approve? ]
      
      ;;calculate percentage
      set opinion-level total-agree / neighbors-num
      
      ;; if there are neighbors (and there are always supposed to be neighbors), you change your opinion in conformity with your neighbors' opinions. 
      if neighbors-num != 0
      [ set opinion-level total-agree / neighbors-num 
      
      ;; so, if more than 50% approve, you do too, otherwise, you follow them along
        ifelse opinion-level >= 0.5 
        [ set temp-approve? TRUE]
        [ set temp-approve? FALSE]
       
       ;;change colors if reconsidered and changed their minds
    ...
    ;;if you want to color in those people who have reconsidered and changed their minds or not, turn on these options here
    ;;  if reconsider? and changed? [ set color red ]
    ;; if reconsider? and not changed? [ set color green ]
       
    ]
  
  ;; a different iteration of the same variables, ignore
  ...
  ;;count how many turtles reconsidered their preferences and changed their minds OR reconsidered and did not change their minds
...
Blue represents people who agree with a policy. White represents people who do not agree.
